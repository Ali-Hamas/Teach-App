...
export const updateBuilding = async (id, updatedData) => {
  ...
  buildings[buildingIndex] = { ...buildings[buildingIndex], ...updatedData, id: parseInt(id) };
  localStorage.setItem('buildings', JSON.stringify(buildings));
...
export const addImageToBuilding = async (buildingId, image) => {
  ...
  const newImage = { ...image, id: generateId(buildings[buildingIndex].images) };
...
export const addPdfToBuilding = async (buildingId, pdf) => {
  ...
  const newPdf = { ...pdf, id: generateId(buildings[buildingIndex].techPDFs) };
